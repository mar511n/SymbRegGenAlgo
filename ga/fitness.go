...
// EvaluatePostfix directly computes a postfix expression without allocations other than a stack.
func EvaluatePostfix(p symbolic.Postfix, vars map[string]float64) (float64, error) {
	...
	for _, token := range p {
		...
		case symbolic.TokenTypeVariable:
			stack = append(stack, vars[token.Name]) // defaults to 0 if not present, or setup check
